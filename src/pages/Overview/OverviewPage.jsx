    ...
    <div id='overview' role='tabpanel' aria-labelledby='tab-1' tabIndex='0'>
      ...
      <div className="wrapper">
        <Card id='balance' title='Current Balance' type='dark'>
          <p className='txt-size-xl'><b>${data.balance.current}</b></p>
        </Card>
        <Card id='income' title='Income'>
          <p className='txt-size-xl'><b>${data.balance.income}</b></p>
        </Card>
        <Card id='expenses' title='Expenses'>
          <p className='txt-size-xl'><b>${data.balance.expenses}</b></p>
        </Card>
        <Card id='pots' title='Pots'>
          <div style={{display: 'flex', alignItems: 'stretch', gap: '2rem'}}>
            <div className='block'>
              ...
              <p>
                Total Saved: <b>${data.pots.reduce((acc, pot) => {
                  acc += pot.total
                  return acc
                }, 0)}</b>
              </p>
            ...
            <ul className='card-list'>
              {
                data.pots.map(pot => {
                  return (
                    <li style={{borderLeft: `4px solid ${pot.theme}`}} key={`pot-${pot.id}`}>{pot.name}: <b>${pot.total}</b></li>
                  )
                })
              }
            </ul> 
